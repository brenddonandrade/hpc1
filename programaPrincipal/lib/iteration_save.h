...
void iteration(int  **M){
	...
	//nao ficou bom
	//#pragma omp parallel for private(itr, i, j, k, l) shared(m_before, m_after, aux_x, aux_y) schedule(dynamic ,chunk) 
	for(itr =0; itr < STEPS ; itr++) {
		...
		//#pragma omp parallel for private(k) shared(aux_x, m_after)
		...
		//#pragma omp parallel for private(k) shared(aux_y, m_after)
		...
		//#pragma omp parallel for private(k, l) shared (m_before, m_after)
