...
void iteration(int  **M){
	...
	//#pragma omp parallel for private(i, j) shared(m_after) reduction(+:cu)
	...
	//nao ficou bom
	//#pragma omp parallel private(itr, i, j, k, l) shared(m_before, m_after, aux_x, aux_y, chunk)
	for(itr =0; itr < STEPS ; itr++) {

        	//#pragma omp parallel for private(i,j) shared(m_after) reduction(+:h)
	        for(i=1; i < (POPULATION_X-1) ; i++)
                	for(j=1 ; j < (POPULATION_Y-1); j++)
        	                h += m_after[i][j];
		...
		//#pragma omp parallel for private(i, j) shared(m_before, m_after)
		//#pragma omp parallel for ordered private(i, j) shared(m_before, m_after) 
		//#pragma omp parallel for private(i, j) shared(m_before, m_after) schedule(dynamic ,chunk) 
		#pragma omp parallel for private(i, j) shared(m_before, m_after) schedule(dynamic, chunk) 
		...
		// cima para baixo e baixo para cima (condicoes periodicas de contorno
		//#pragma omp parallel for private(k) shared(aux_x, m_after) schedule(dynamic, chunk)
		...
		//#pragma omp parallel for private(k) shared(aux_y, m_after) schedule(dynamic, chunk)
		...
		//#pragma omp parallel for private(k, l) shared (m_before, m_after) schedule(dymanic, chunk)
